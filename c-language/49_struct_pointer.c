...
int main491()
{
    ...
    strcpy(p1->name, "홍길동"); //  p1->name 등의 문자열 멤버는 = (할당 연산자)로 저장할 수 없으므로 strcpy 함수를 사용하면 됩니다.
    p1->age = 30;
    strcpy(p1->address, "서울시 용산구 한남동");

    // 화살표 연산자로 구조체 멤버에 접근하여 값 출력
    printf("이름: %s\n", p1->name);       // 홍길동
    printf("나이: %d\n", p1->age);        // 30  p1->age;      // 화살표 연산자로 멤버에 접근
    printf("나이: %d\n", (*p1).age);        // 30 (*p1).age;    // 구조체 포인터를 역참조한 뒤 .으로 멤버에 접근
